...
const cases = [
  ...
  },
  {
    ...
    date: '05 April 2021',
    catchwords: 'TORT - Negligence - Workplace injury - Duty of care - Construction site safety - Contributory negligence - Quantum of damages.',
    ...
    topic: 'workplace negligence and construction safety standards'
  }
];
...
async function generatePDFs() {
  for (const caseData of cases) {
    ...
    // PAGE 2-3: Procedural History & Background
    doc.addPage();
    doc.font('Helvetica-Bold').fontSize(14).text('1. PROCEDURAL HISTORY', { underline: true });
    doc.moveDown();
    doc.font('Helvetica').fontSize(11).text(generateLegalFiller(5, caseData.topic), { align: 'justify' });
    doc.moveDown();
    
    doc.font('Helvetica-Bold').fontSize(14).text('2. BACKGROUND AND MATERIAL FACTS', { underline: true });
    doc.moveDown();
    doc.font('Helvetica').fontSize(11).text(caseData.facts, { align: 'justify' });
    doc.moveDown();
    doc.text(generateLegalFiller(6, caseData.topic), { align: 'justify' });

    // PAGE 4-5: Submissions
    doc.addPage();
    doc.font('Helvetica-Bold').fontSize(14).text('3. SUBMISSIONS OF THE PLAINTIFF / APPLICANT', { underline: true });
    doc.moveDown();
    doc.font('Helvetica').fontSize(11).text(generateLegalFiller(4, caseData.topic), { align: 'justify' });
    doc.moveDown();

    doc.font('Helvetica-Bold').fontSize(14).text('4. SUBMISSIONS OF THE DEFENDANT / RESPONDENT', { underline: true });
    doc.moveDown();
    doc.font('Helvetica').fontSize(11).text(generateLegalFiller(4, caseData.topic), { align: 'justify' });

    // PAGE 6-7: Legal Framework and Judgment
    doc.addPage();
    doc.font('Helvetica-Bold').fontSize(14).text('5. LEGAL FRAMEWORK AND PRECEDENTS', { underline: true });
    doc.moveDown();
    doc.font('Helvetica').fontSize(11).text(generateLegalFiller(5, caseData.topic), { align: 'justify' });
    doc.moveDown();

    doc.font('Helvetica-Bold').fontSize(14).text('6. JUDGMENT AND REASONING', { underline: true });
    doc.moveDown();
    doc.font('Helvetica').fontSize(11).text(caseData.judgment, { align: 'justify' });
    doc.moveDown();
    doc.text(generateLegalFiller(4, caseData.topic), { align: 'justify' });

    // PAGE 8: Orders
    doc.addPage();
    doc.font('Helvetica-Bold').fontSize(14).text('7. FINAL ORDERS', { underline: true });
    doc.moveDown();
    ...
    console.log(`Generated multi-page document: ${caseData.filename}`);
  }
}

generatePDFs().then(() => console.log('All long sample PDFs generated successfully.'));
